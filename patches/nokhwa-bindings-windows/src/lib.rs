...
pub mod wmf {
    ...
    use windows::Win32::Media::KernelStreaming::{
        KSCAMERA_EXTENDEDPROP_EVCOMPENSATION, KSCAMERA_EXTENDEDPROP_HEADER,
        KSPROPERTY_CAMERACONTROL_EXTENDED_EVCOMPENSATION,
    };
    use windows::Win32::Media::MediaFoundation::{
        IMFExtendedCameraControl, IMFExtendedCameraController, IMFGetService,
        MF_CAPTURE_ENGINE_MEDIASOURCE, MFCreateSample, MF_SOURCE_READER_FIRST_VIDEO_STREAM,
    };
    use windows::{
    ...
    /// Probed extended EV compensation control (auto exposure bias).
    #[derive(Debug, Clone, Copy, PartialEq, Eq)]
    pub struct EvCompProbe {
        pub min: i32,
        pub max: i32,
        pub value: i32,
        pub step_flags: u64,
    ...
    /// Diagnostic result from probing extended EV compensation.
    #[derive(Debug, Clone)]
    pub struct EvCompProbeDiagnostic {
        pub media_source_controller: Option<String>,
    ...
    impl MediaFoundationDevice {
        ...
        fn iam_camera_control(&self) -> Result<IAMCameraControl, NokhwaError> {
            unsafe {
                let mut receiver: MaybeUninit<IAMCameraControl> = MaybeUninit::uninit();
                let ptr_receiver = receiver.as_mut_ptr();
                if let Err(why) = self.source_reader.GetServiceForStream(
        ...
        fn iam_video_proc_amp(&self) -> Result<IAMVideoProcAmp, NokhwaError> {
            unsafe {
                let mut receiver: MaybeUninit<IAMVideoProcAmp> = MaybeUninit::uninit();
                let ptr_receiver = receiver.as_mut_ptr();
                if let Err(why) = self.source_reader.GetServiceForStream(
        ...
        fn extended_camera_controller_from_media_source(&self) -> Result<IMFExtendedCameraController, String> {
            unsafe {
                let get_service = self
                    .media_source
                    .cast::<IMFGetService>()
                    .map_err(|e| e.to_string())?;
                get_service
                    .GetService::<IMFExtendedCameraController>(&GUID_NULL)
                    .map_err(|e| e.to_string())
            }
        }

        fn extended_camera_controller_from_source_reader(&self) -> Result<IMFExtendedCameraController, String> {
            unsafe {
                let mut receiver: MaybeUninit<IMFExtendedCameraController> = MaybeUninit::uninit();
                let ptr_receiver = receiver.as_mut_ptr();
                self.source_reader
                    .GetServiceForStream(
                        MF_SOURCE_READER_MEDIASOURCE,
                        &GUID_NULL,
                        &IMFExtendedCameraController::IID,
                        ptr_receiver
                            .cast::<IMFExtendedCameraController>()
                            .cast::<*mut c_void>(),
                    )
                    .map_err(|e| e.to_string())?;
                Ok(receiver.assume_init())
        ...
        fn extended_camera_controller(&self) -> Option<IMFExtendedCameraController> {
            self.extended_camera_controller_from_media_source()
                .ok()
                .or_else(|| self.extended_camera_controller_from_source_reader().ok())
        }

        fn extended_ev_control(&self) -> Option<IMFExtendedCameraControl> {
            let controller = self.extended_camera_controller()?;
            unsafe {
                controller
                    .GetExtendedCameraControl(
                        MF_CAPTURE_ENGINE_MEDIASOURCE.0,
                        KSPROPERTY_CAMERACONTROL_EXTENDED_EVCOMPENSATION.0 as u32,
                    )
                    .ok()
            }
        }

        fn extended_ev_control_with_diagnostic(
            &self,
            diagnostic: &mut EvCompProbeDiagnostic,
        ) -> Option<IMFExtendedCameraControl> {
            match self.extended_camera_controller_from_media_source() {
                Ok(controller) => {
                    diagnostic.media_source_controller = Some("OK".to_string());
                    unsafe {
                        return controller
                            .GetExtendedCameraControl(
                                MF_CAPTURE_ENGINE_MEDIASOURCE.0,
                ...
                Err(error) => {
                    diagnostic.media_source_controller = Some(error);
                }
            }

            match self.extended_camera_controller_from_source_reader() {
                Ok(controller) => {
                    diagnostic.source_reader_controller = Some("OK".to_string());
                    unsafe {
                        return controller
                            .GetExtendedCameraControl(
                                MF_CAPTURE_ENGINE_MEDIASOURCE.0,
                ...
                Err(error) => {
                    diagnostic.source_reader_controller = Some(error);
                }
            }
        ...
        unsafe fn read_ev_comp_payload(
            payload_ptr: *mut u8,
            payload_len: u32,
            control_flags: u64,
        ) -> Option<EvCompProbe> {
        ...
        /// Probe `KSPROPERTY_CAMERACONTROL_EXTENDED_EVCOMPENSATION` (true auto-exposure bias).
        pub fn probe_ev_compensation(&self) -> Option<EvCompProbe> {
            self.probe_ev_compensation_diagnostic().probe
        }

        /// Probe EV compensation with per-stage diagnostics for verbose logging.
        pub fn probe_ev_compensation_diagnostic(&self) -> EvCompProbeDiagnostic {
            let mut diagnostic = EvCompProbeDiagnostic {
                media_source_controller: None,
            ...
            let Some(ev_control) = self.extended_ev_control_with_diagnostic(&mut diagnostic) else {
                diagnostic.get_extended_control =
                    Some("GetExtendedCameraControl failed on all paths".to_string());
                return diagnostic;
            };
            diagnostic.get_extended_control = Some("OK".to_string());

            let mut payload_ptr: *mut u8 = std::ptr::null_mut();
            let mut payload_len: u32 = 0;

            unsafe {
                if let Err(error) = ev_control.LockPayload(&mut payload_ptr, &mut payload_len) {
        ...
        /// Set EV compensation in driver step units while keeping auto exposure active.
        pub fn set_ev_compensation_steps(&self, value: i32) -> Result<(), NokhwaError> {
            let ev_control = self.extended_ev_control().ok_or(NokhwaError::SetPropertyError {
                property: "EVCompensation".to_string(),
                value: value.to_string(),
                error: "extended EV compensation not supported".to_string(),
            })?;
            ...
            unsafe {
                ev_control
                    .LockPayload(&mut payload_ptr, &mut payload_len)
                    .map_err(|e| NokhwaError::SetPropertyError {
                        property: "EVCompensation".to_string(),
                        value: value.to_string(),
                        error: e.to_string(),
                    })?;
        ...
        /// Restore legacy auto exposure (manual fallback reset).
        pub fn set_exposure_auto(&self) -> Result<(), NokhwaError> {
            let camera_control = self.iam_camera_control()?;
            let mut value = 0;
            let mut flag = 0;
            unsafe {
                if let Err(why) =
                ...
                if let Err(why) = camera_control.Set(
        ...
        /// Set absolute manual shutter exposure (manual fallback only).
        pub fn set_exposure_manual(&self, value: i32) -> Result<(), NokhwaError> {
            let camera_control = self.iam_camera_control()?;
            unsafe {
                if let Err(why) = camera_control.Set(
                    CameraControl_Exposure.0,
        ...
        /// Restore auto gain (gain-bias fallback reset).
        pub fn set_gain_auto(&self) -> Result<(), NokhwaError> {
            let video_proc_amp = self.iam_video_proc_amp()?;
            let mut value = 0;
            let mut flag = 0;
            unsafe {
                if let Err(why) = video_proc_amp.Get(VideoProcAmp_Gain.0, &mut value, &mut flag) {
                    return Err(NokhwaError::GetPropertyError {
                ...
                if let Err(why) =
                    video_proc_amp.Set(VideoProcAmp_Gain.0, value, CameraControl_Flags_Auto.0)
                {
                    return Err(NokhwaError::SetPropertyError {
                        property: "Gain".to_string(),
                        value: value.to_string(),
        ...
        /// Set absolute manual gain (gain-bias fallback only).
        pub fn set_gain_manual(&self, value: i32) -> Result<(), NokhwaError> {
            let video_proc_amp = self.iam_video_proc_amp()?;
            unsafe {
                if let Err(why) =
                    video_proc_amp.Set(VideoProcAmp_Gain.0, value, CameraControl_Flags_Manual.0)
                {
                    return Err(NokhwaError::SetPropertyError {
                        property: "Gain".to_string(),
                        value: value.to_string(),
...
pub mod wmf {
    ...
    impl MediaFoundationDevice {
        ...
        pub fn set_ev_compensation_steps(&self, _value: i32) -> Result<(), NokhwaError> {
            Err(NokhwaError::NotImplementedError(
                "Only on Windows".to_string(),
            ))
        }
        ...
        pub fn set_exposure_manual(&self, _value: i32) -> Result<(), NokhwaError> {
            Err(NokhwaError::NotImplementedError(
                "Only on Windows".to_string(),
            ))
        }

        pub fn set_gain_manual(&self, _value: i32) -> Result<(), NokhwaError> {
            Err(NokhwaError::NotImplementedError(
                "Only on Windows".to_string(),
            ))
        }
