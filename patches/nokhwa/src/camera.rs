...
impl Camera {
    ...
    /// Probe extended EV compensation (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn probe_ev_compensation(&self) -> Option<nokhwa_bindings_windows::wmf::EvCompProbe> {
        if self.backend() != ApiBackend::MediaFoundation {
            return None;
        }
        unsafe {
            let device = &*(self.device.as_ref() as *const dyn CaptureBackendTrait
                as *const crate::backends::capture::MediaFoundationCaptureDevice);
            device.probe_ev_compensation()
        }
    }

    /// Probe EV compensation with diagnostics (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn probe_ev_compensation_diagnostic(
        &self,
    ) -> nokhwa_bindings_windows::wmf::EvCompProbeDiagnostic {
        if self.backend() != ApiBackend::MediaFoundation {
            return nokhwa_bindings_windows::wmf::EvCompProbeDiagnostic {
        ...
        unsafe {
            let device = &*(self.device.as_ref() as *const dyn CaptureBackendTrait
                as *const crate::backends::capture::MediaFoundationCaptureDevice);
            device.probe_ev_compensation_diagnostic()
        }
    }

    /// Set EV compensation step units (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn set_ev_compensation_steps(&mut self, value: i32) -> Result<(), NokhwaError> {
        if self.backend() != ApiBackend::MediaFoundation {
            return Err(NokhwaError::NotImplementedError(
                "EV compensation requires MediaFoundation backend".to_string(),
            ));
        }
        unsafe {
            let device = &mut *(self.device.as_mut() as *mut dyn CaptureBackendTrait
                as *mut crate::backends::capture::MediaFoundationCaptureDevice);
            device.set_ev_compensation_steps(value)
        }
    }

    /// Restore auto exposure (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn restore_auto_exposure(&mut self) -> Result<(), NokhwaError> {
        if self.backend() != ApiBackend::MediaFoundation {
            return Err(NokhwaError::NotImplementedError(
                "auto exposure restore requires MediaFoundation backend".to_string(),
            ));
        }
        unsafe {
            let device = &mut *(self.device.as_mut() as *mut dyn CaptureBackendTrait
                as *mut crate::backends::capture::MediaFoundationCaptureDevice);
            device.set_exposure_auto()
        }
    }

    /// Set manual shutter exposure (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn set_exposure_manual(&mut self, value: i32) -> Result<(), NokhwaError> {
        if self.backend() != ApiBackend::MediaFoundation {
            return Err(NokhwaError::NotImplementedError(
                "manual exposure requires MediaFoundation backend".to_string(),
            ));
        }
        unsafe {
            let device = &mut *(self.device.as_mut() as *mut dyn CaptureBackendTrait
                as *mut crate::backends::capture::MediaFoundationCaptureDevice);
            device.set_exposure_manual(value)
        }
    }

    /// Set manual gain (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn set_gain_manual(&mut self, value: i32) -> Result<(), NokhwaError> {
        if self.backend() != ApiBackend::MediaFoundation {
            return Err(NokhwaError::NotImplementedError(
                "manual gain requires MediaFoundation backend".to_string(),
            ));
        }
        unsafe {
            let device = &mut *(self.device.as_mut() as *mut dyn CaptureBackendTrait
                as *mut crate::backends::capture::MediaFoundationCaptureDevice);
            device.set_gain_manual(value)
        }
    }

    /// Restore auto gain (Windows Media Foundation only).
    #[cfg(all(feature = "input-msmf", target_os = "windows"))]
    pub fn restore_auto_gain(&mut self) -> Result<(), NokhwaError> {
        if self.backend() != ApiBackend::MediaFoundation {
            return Err(NokhwaError::NotImplementedError(
                "auto gain restore requires MediaFoundation backend".to_string(),
            ));
        }
        unsafe {
            let device = &mut *(self.device.as_mut() as *mut dyn CaptureBackendTrait
