...
impl MediaFoundationCaptureDevice {
    ...
    /// Probe extended EV compensation (auto-exposure bias).
    pub fn probe_ev_compensation(&self) -> Option<nokhwa_bindings_windows::wmf::EvCompProbe> {
        self.inner.probe_ev_compensation()
    }

    /// Probe EV compensation with per-stage diagnostics.
    pub fn probe_ev_compensation_diagnostic(
        &self,
    ) -> nokhwa_bindings_windows::wmf::EvCompProbeDiagnostic {
        self.inner.probe_ev_compensation_diagnostic()
    }

    /// Set EV compensation in driver step units.
    pub fn set_ev_compensation_steps(&self, value: i32) -> Result<(), NokhwaError> {
        self.inner.set_ev_compensation_steps(value)
    }

    /// Restore legacy auto exposure.
    pub fn set_exposure_auto(&self) -> Result<(), NokhwaError> {
        self.inner.set_exposure_auto()
    }

    /// Set absolute manual shutter exposure.
    pub fn set_exposure_manual(&self, value: i32) -> Result<(), NokhwaError> {
        self.inner.set_exposure_manual(value)
    }

    /// Set absolute manual gain.
    pub fn set_gain_manual(&self, value: i32) -> Result<(), NokhwaError> {
        self.inner.set_gain_manual(value)
    }

    /// Restore auto gain.
    pub fn set_gain_auto(&self) -> Result<(), NokhwaError> {
        self.inner.set_gain_auto()
    }
}
