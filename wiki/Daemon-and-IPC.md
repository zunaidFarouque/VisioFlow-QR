...
Transport: **named pipes** on Windows, **Unix domain sockets** on Linux.
...
> **Note:** The wire protocol uses explicit `execute_rule` with a rule name. Auto-route on IPC is achieved by client-side resolution (same core routing API), then forwarding the matched rule to the daemon. A dedicated IPC auto-route message is not yet implemented.
