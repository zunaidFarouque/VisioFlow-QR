...
**Toast Copy button:** Routing toasts (`--notify`) and `notify test` include a **Copy** button (or **Copy again** when routing already copied the payload to the clipboard). Clicking it launches headless `visioflow-toast.exe` (Windows GUI subsystem, no console flash) via the registered `visioflow:` protocol URL, reads the **full raw payload** from a temp file in `%TEMP%` (not the truncated toast body), and copies it to the clipboard. VisioFlow registers the protocol handler and a stub toast-activator CLSID on the Start Menu shortcut automatically on first toast send — rebuild so `visioflow-toast.exe` sits beside `visioflow.exe`.
