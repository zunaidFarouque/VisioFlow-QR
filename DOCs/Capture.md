...
The `capture` command decodes visual payloads from a **screen snip** or **webcam**, then optionally routes them through rules. This page documents sources, filters, webcam tuning, halts, and feedback flags.
...
When `--trigger` is omitted and `--action` is omitted, capture **auto-routes** after decode (see [Routing-And-Default-Rules.md](Routing-And-Default-Rules.md)).
...
Release zips and Scoop installs ship `models/` next to the binary. Scoop sets `VISIOFLOW_MODELS_DIR` to `$dir\models`. Dev builds can run `scripts/download-wechat-models.ps1` — see [`models/README.md`](../models/README.md).
...
# Quiet console toasts
visioflow capture --source snip --no-notify
```

Notifications: [Notifications-Windows.md](Notifications-Windows.md).
...
| `--ipc-socket <PATH>` | Route via daemon (see [Daemon-and-IPC.md](Daemon-and-IPC.md)) |
...
| Toast missing | [Notifications-Windows.md](Notifications-Windows.md) |

## Related

- [`Encode.md`](Encode.md) — generate QR from clipboard (inverse of capture)
- [`Routing-And-Default-Rules.md`](Routing-And-Default-Rules.md) — auto-route after decode
